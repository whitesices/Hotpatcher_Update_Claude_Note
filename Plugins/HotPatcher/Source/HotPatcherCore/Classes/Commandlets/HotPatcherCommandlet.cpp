...
int32 UHotPatcherCommandlet::Main(const FString& Params)
{
	...
	// Reset the cook commandlet flag before engine shutdown.
	// UWorldPartition::BeginDestroy checks IsRunningCookCommandlet() and calls
	// UnregisterCookPackageSubSplitterFactory when true. But the sub-splitter was
	// never registered (CDO constructed before we set this flag), so the unregister
	// check assertion fails during GC on engine exit. Resetting to false prevents this.
#if WITH_UE5
