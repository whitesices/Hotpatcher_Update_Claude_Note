...
#endif
	// Skip ALL map packages when UseCookCmdlet is disabled (UE5.6+).
	// HotPatcherCommandlet sets PRIVATE_GIsRunningCookCommandlet = true in Main(),
	// but by that time UWorldPartition's CDO has already been constructed (when the
	// flag was still false), so the WorldPartition sub-splitter factory was never
	// registered via RegisterCookPackageSubSplitterFactory. Saving ANY map through
	// GEditor->Save() in this state triggers WorldCookPackageSplitter assertion
	// (check(CookPackageSubSplitters.Num()) in ReportGenerationManifest, or
	// check(RegisteredCookPackageSubSplitterFactories.Contains(Class)) in Unregister).
	// The only safe approach is to skip all map packages on UE5.6+ when the cook
	// cmdlet pipeline is not available.
#if !UE_VERSION_OLDER_THAN(5,6,0)
	if (IsValid(Package) && Package->ContainsMap())
	{
		FString PackageName = Package->GetPathName();
		UE_LOG(LogHotPatcher, Warning,
			TEXT("Skip cooking map package \"%s\": HotPatcher direct cook path does not support "
				"map packages on UE5.6+ (CookPackageSubSplitter factories not registered). "
				"Use UE's standard packaging pipeline for maps."),
			*PackageName);
		// Notify callbacks that the cook was skipped
		if (CookActionCallback.OnCookBegin)
		{
			FString LongPackageName = UFlibAssetManageHelper::PackagePathToLongPackageName(PackageName);
			for (const auto& Platform : CookPlatforms)
			{
				CookActionCallback.OnCookBegin(LongPackageName, Platform.Key);
			}
		}
		if (CookActionCallback.OnAssetCooked)
		{
			...
			for (const auto& Platform : CookPlatforms)
			{
				CookActionCallback.OnAssetCooked(FSoftObjectPath(LongPackageName), Platform.Key, ESavePackageResult::Error);
			}
		}
		return false;
	}
